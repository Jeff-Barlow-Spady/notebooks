...
def update_success_pie_chart(selected_site):
    if selected_site == "ALL":
        # Aggregate data by launch site for all launches
        fig = px.pie(
            spacex_df,
            names="Launch Site",
            title="Total Successful Launches by Site",
            values='class'  # Ensure 'class' or any suitable value column is provided
        )
    else:
        # Filter data for the selected launch site and aggregate by success
        site_df = spacex_df[spacex_df["Launch Site"] == selected_site]
        fig = px.pie(
            site_df,
            names="class",
            title=f"Successful Launches for {selected_site}",
            values=site_df['class'].value_counts()  # Assuming 'class' indicates success/failure
        )
    
    # Update the layout to include the legend title and other properties
    fig.update_traces(textposition="inside", textinfo="percent+label")
    fig.update_layout(
        showlegend=True,
        margin=dict(t=40, b=40, l=40, r=40),
        legend=dict(title='Launch Outcome', x=0, y=1, traceorder='normal'),  # Set the legend title here
        uniformtext_minsize=12, uniformtext_mode='hide'  # Improve text visibility
    )
    return fig
